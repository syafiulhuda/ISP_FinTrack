...
export function PaymentModal({ isOpen, onClose, token, redirectUrl, orderId }: PaymentModalProps) {
 ...
 <div className="w-full h-full max-h-[85vh] lg:h-auto lg:max-w-[520px] bg-card flex flex-col overflow-hidden rounded-2xl lg:rounded-3xl border border-border shadow-2xl relative">

 {/* Floating Close Button - TETAP */}
 <button
 onClick={handleClose}
 className="absolute top-3 right-3 z-[60] w-8 h-8 flex items-center justify-center bg-muted/90 hover:bg-muted/90 text-muted-foreground rounded-full backdrop-blur-md shadow-sm transition-all"
 ...
 <div className="flex-1 w-full bg-background relative overflow-y-auto overflow-x-hidden scrollable-area">
 {isLoading && (
 <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-muted/80 /80 z-10">
 <div className="w-8 h-8 border-4 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin"/>
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Loading secure payment screen...</p>
 </div>
 )}
