...
export function PwaInstallButton() {
 ...
 <AnimatePresence>
 {showIOSPrompt && (
 <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-card/40 backdrop-blur-sm"onClick={() => setShowIOSPrompt(false)}>
 <m.div
 initial={{ y:"100%", opacity: 0 }}
 animate={{ y: 0, opacity: 1 }}
 exit={{ y:"100%", opacity: 0 }}
 className="bg-card p-6 rounded-t-[2rem] sm:rounded-[2rem] shadow-2xl w-full max-w-sm border border-border"
 onClick={e => e.stopPropagation()}
 >
 <h3 className="font-black text-xl mb-6 text-center tracking-tight">Install on iOS</h3>
 <div className="flex flex-col items-center space-y-4 text-sm font-medium text-muted-foreground">
 <p className="text-center w-full mb-2 text-muted-foreground">To install this app on your iPhone or iPad, follow these simple steps:</p>
 <div className="flex items-center gap-4 bg-muted px-5 py-4 rounded-2xl w-full border border-border">
 <span className="bg-white shadow-sm text-foreground w-8 h-8 rounded-full flex items-center justify-center font-black shrink-0">1</span>
 <span>Tap the <Share size={20} className="inline mx-1 text-blue-500"/> <strong>Share</strong> icon on the Safari bottom bar.</span>
 </div>
 <div className="flex items-center gap-4 bg-muted px-5 py-4 rounded-2xl w-full border border-border">
 <span className="bg-white shadow-sm text-foreground w-8 h-8 rounded-full flex items-center justify-center font-black shrink-0">2</span>
 <span>Scroll down and select <strong>"Add to Home Screen"</strong> <PlusSquare size={18} className="inline mx-1 text-muted-foreground"/></span>
 </div>
 <button 
