...
export function Topbar({ onMenuClick }: { onMenuClick?: () => void }) {
 ...
 return (
 <header className="fixed top-0 right-0 left-0 md:left-64 z-50 bg-muted/80 backdrop-blur-xl shadow-[0px_12px_32px_rgba(25,28,30,0.06)] flex items-center justify-between px-4 md:px-8 h-16 transition-all duration-300">
 ...
 className="p-2 rounded-xl bg-muted/50 text-muted-foreground hover:bg-muted transition-all duration-300 shadow-sm border border-border/50"
 title="Toggle theme"
 ...
 <Link href="/notifications"className="relative text-muted-foreground hover:bg-muted dark:hover:bg-muted transition-colors p-2 rounded-full active:opacity-80"aria-label="View notifications">
 <Bell size={20} />
 {unreadCount > 0 && (
 ...
 <Link href="/settings"className="text-muted-foreground hover:bg-muted dark:hover:bg-muted transition-colors p-2 rounded-full active:opacity-80"aria-label="Settings">
 <Settings size={20} />
 </Link>
 <Link href="/profile"className="w-8 h-8 rounded-full overflow-hidden border border-border ml-1 tablet:ml-2 block shrink-0"aria-label="View user profile">
 <Image
 unoptimized
 width={32}
 height={32}
