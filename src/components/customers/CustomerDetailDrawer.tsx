...
function StatCell({ label, value, icon: Icon, accent }: { label: string; value: string; icon: any; accent: string }) {
 return (
 <div className="flex flex-col gap-1.5 p-3 bg-white rounded-xl border border-border">
...
export function CustomerDetailDrawer({ customer, onClose }: CustomerDetailDrawerProps) {
 ...
 className="absolute right-0 top-0 bottom-0 w-full max-w-[440px] bg-white shadow-[-8px_0_32px_rgba(0,0,0,0.12)] dark:shadow-[-8px_0_32px_rgba(0,0,0,0.4)] flex flex-col overflow-hidden border-l border-border pointer-events-auto"
 >
 ...
 <div>
 <h2 className="text-base font-black text-foreground">{customer.name}</h2>
 <p className="text-xs font-bold text-muted-foreground">{customer.id}</p>
 ...
 className="p-1.5 text-muted-foreground hover:text-foreground dark:hover:text-white hover:bg-muted dark:hover:bg-muted rounded-xl transition-all">
 <X size={18} />
 </button>
 </div>
 </div>
 ...
 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3">Health Score Breakdown</p>
 ...
 {/* Financial Stats */}
 <div>
 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2">Financial Overview</p>
 ...
 {/* Payment Performance */}
 <div className="bg-card rounded-2xl p-4">
 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3">Payment Performance</p>
 <div className="space-y-2.5">
 <div className="flex justify-between items-center">
 <span className="text-[11px] font-bold text-muted-foreground">On-Time Rate</span>
 ...
 <div className="text-center p-2 bg-white rounded-xl border border-border">
 <p className="text-sm font-black text-emerald-500">{onTimeCount}</p>
 <p className="text-[9px] font-bold text-muted-foreground">Tepat Waktu</p>
 </div>
 <div className="text-center p-2 bg-white rounded-xl border border-border">
 <p className={cn("text-sm font-black", lateCount > 0 ?"text-rose-500":"text-muted-foreground")}>{lateCount}</p>
 <p className="text-[9px] font-bold text-muted-foreground">Terlambat</p>
 </div>
 </div>
 ...
 <div className="bg-card rounded-2xl p-4">
 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3">Payment Timeline</p>
 ...
 <div 
 className="bg-card/95 backdrop-blur-xl text-white px-4 py-3 rounded-2xl text-xs font-black shadow-2xl border border-white/10 ring-1 ring-white/5 transition-transform duration-200"
 style={{ 
 ...
 <div className="bg-card rounded-2xl p-4">
 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3">Late Payments History</p>
 <div className="overflow-hidden rounded-xl border border-border">
 <table className="w-full text-left">
 <thead className="bg-muted/50">
 <tr>
 <th className="px-3 py-2 text-[10px] font-bold text-muted-foreground">Month</th>
 <th className="px-3 py-2 text-[10px] font-bold text-muted-foreground">Days Late</th>
 <th className="px-3 py-2 text-[10px] font-bold text-muted-foreground text-right">Amount</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
 {data.late_payments.map((lp: any, i: number) => (
 <tr key={i} className="bg-white">
 <td className="px-3 py-2 text-[11px] font-bold text-foreground">{lp.month}</td>
 <td className="px-3 py-2 text-[11px] font-black text-rose-500">{lp.daysLate} days</td>
 <td className="px-3 py-2 text-[11px] font-bold text-foreground text-right">{formatCurrency(lp.amount)}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 {/* CRM Intelligence */}
 <div className="bg-card rounded-2xl p-4">
 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3">CRM Intelligence</p>
 <div className="space-y-2">
 <div className="flex justify-between items-center py-1.5 border-b border-border">
 <span className="flex items-center gap-2 text-[11px] font-bold text-muted-foreground"><Tag size={11} /> Segment</span>
 <span className={cn("text-[10px] font-black px-2 py-0.5 rounded-lg", segmentColor)}>{segment}</span>
 </div>
 <div className="flex justify-between items-center py-1.5 border-b border-border">
 <span className="flex items-center gap-2 text-[11px] font-bold text-muted-foreground"><BarChart2 size={11} /> Churn Probability</span>
 <span className={cn("text-[11px] font-black", churnColor)}>{churnPct}</span>
 </div>
 {tenureMonths !== null && (
 <div className="flex justify-between items-center py-1.5 border-b border-border">
 <span className="flex items-center gap-2 text-[11px] font-bold text-muted-foreground"><Milestone size={11} /> Tenure</span>
 <span className="text-[11px] font-black text-foreground">{tenureMonths} bulan</span>
 </div>
 )}
 <div className="flex justify-between items-center py-1.5">
 <span className="flex items-center gap-2 text-[11px] font-bold text-muted-foreground"><Package size={11} /> Paket Aktif</span>
 <span className="text-[11px] font-black text-foreground">{data.service}</span>
 </div>
 </div>
 </div>
 ...
 <button onClick={onClose}
 aria-label="Close customer drawer"
 className="w-full py-2.5 bg-muted hover:bg-muted text-muted-foreground rounded-xl font-black text-sm transition-all cursor-pointer">
 Tutup
 </button>
 </div>
 </m.div>
