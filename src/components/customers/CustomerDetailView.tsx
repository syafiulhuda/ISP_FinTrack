...
function HealthScoreGauge({ score }: { score: number }) {
 ...
 return (
 <div className="flex flex-col items-center gap-3 tablet:gap-4 bg-card p-6 tablet:p-8 rounded-3xl border border-border shadow-sm">
 ...
 <span className="text-[10px] tablet:text-xs font-bold text-muted-foreground">HEALTH SCORE</span>
 </div>
 </div>
...
export default function CustomerDetailView({ data, initialTickets = [] }: { data: any, initialTickets?: any[] }) {
 ...
 <Link href="/customers"aria-label="Back to customer list"className="p-2.5 tablet:p-3 bg-card rounded-2xl border border-border hover:scale-105 transition-transform shrink-0 mt-1 tablet:mt-0">
 <ChevronLeft size={18} className="text-muted-foreground"/>
 </Link>
 <div className="flex-1 min-w-0">
 <div className="flex flex-wrap items-center gap-2">
 <h1 className="text-xl sm:text-2xl tablet:text-3xl font-black text-foreground tracking-tight flex items-center gap-2 truncate">
 {data.name}
 {isVip && (
 <m.div
 initial={{ scale: 0 }}
 ...
 <div>
 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Customer Segment</p>
 <p className={cn("text-base sm:text-lg font-black", segmentColor.split('')[0])}>{segment}</p>
 </div>
 <div>
 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Churn Risk Level</p>
 ...
 <div className="pt-4 border-t border-border flex flex-row items-center justify-between gap-3">
 ...
 <Phone size={12} className="text-muted-foreground shrink-0"/> {data.no_telp ||'N/A'}
 </p>
 </div>
 <div className="text-right shrink-0">
 <p className="text-[10px] font-bold text-muted-foreground">Region</p>
 <p className="text-xs font-black dark:text-white flex items-center justify-end gap-1.5 mt-0.5 whitespace-nowrap">
 <MapPin size={12} className="text-muted-foreground shrink-0"/> {data.city}
 </p>
 </div>
 </div>
 </div>
 ...
 <div className="bg-card/95 backdrop-blur-xl text-white px-3 md:px-6 py-2 md:py-4 rounded-2xl shadow-2xl border border-white/10 ring-1 ring-white/5 min-w-[140px] md:min-w-[240px]">
 ...
 <div className="w-full h-full bg-muted/5 animate-pulse rounded-2xl border border-border/10 /10"/>
 )}
 </div>
 </div>
 ...
 >
 <h2 className="text-base sm:text-lg font-black flex items-center gap-3 group-hover:text-muted-foreground dark:group-hover:text-muted-foreground transition-colors">
 <AlertCircle size={20} className="text-rose-500 shrink-0"/>
 <span>Ticket History</span>
 </h2>
 <ChevronDown size={16} className={cn("text-muted-foreground transition-transform", isTicketHistoryExpanded &&"rotate-180")} />
 </div>
 <button
 ...
 <div className="text-center p-6 border border-dashed border-border rounded-2xl text-muted-foreground text-sm font-medium">
 No tickets found for this customer.
 </div>
 ) : (
 <>
 {paginatedTickets.map((t) => (
 <div key={t.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 border border-border rounded-2xl hover:bg-muted dark:hover:bg-muted/50 transition-colors">
 <div>
 <div className="flex items-center gap-2 mb-1">
 <span className="text-xs font-black text-foreground">{t.ticket_number}</span>
...
'bg-emerald-500/10 text-emerald-500'
 ...
 className="p-1.5 rounded-lg bg-muted text-muted-foreground hover:bg-muted disabled:opacity-50 transition-colors"
 >
 <ChevronLeft size={16} />
 </button>
 <button
 ...
 <div className="bg-card p-4 sm:p-8 rounded-3xl border border-border shadow-sm">
 ...
 <tr>
 <th className="px-6 py-4 text-xs font-black text-muted-foreground uppercase tracking-widest">Billing Month</th>
 <th className="px-6 py-4 text-xs font-black text-muted-foreground uppercase tracking-widest">Days Delayed</th>
 <th className="px-6 py-4 text-xs font-black text-muted-foreground uppercase tracking-widest">Reference Date</th>
 <th className="px-6 py-4 text-xs font-black text-muted-foreground uppercase tracking-widest text-right">Payment Amount</th>
 </tr>
 </thead>
...
"px-3 py-1 rounded-lg text-sm font-black",
 ...
 <div className="block sm:hidden divide-y divide-slate-100 dark:divide-slate-800 border border-border rounded-2xl overflow-hidden">
 {data.late_payments && data.late_payments.length > 0 ? (
 paginatedLatePayments.map((lp: any, i: number) => {
...
"p-4 space-y-4 transition-colors",
 lp.isUnpaid ?"bg-rose-500/5 dark:bg-rose-500/10":"bg-card"
 )}
 >
 ...
 <span className="font-black text-foreground text-sm group-hover:text-rose-500 transition-colors">
 {lp.month}
 </span>
 {lp.isUnpaid && (
...
"px-2.5 py-0.5 rounded-lg text-[10px] font-black shrink-0",
 ...
 className="overflow-hidden"
 >
 <div className="pt-2 pb-1 grid grid-cols-2 gap-3 text-xs border-t border-border/60 mt-3">
 <div className="bg-muted/40 p-3 rounded-xl border border-border/60 text-left">
 <span className="block text-[8px] font-black text-muted-foreground uppercase tracking-widest mb-1">Reference Date</span>
 <span className="font-bold text-muted-foreground">
 {lp.isUnpaid ?"Belum Terdeteksi": new Date(lp.date).toLocaleDateString('id-ID', { day:'numeric', month:'long', year:'numeric'})}
 </span>
 </div>
 <div className="bg-muted/40 p-3 rounded-xl border border-border/60 text-left">
 <span className="block text-[8px] font-black text-muted-foreground uppercase tracking-widest mb-1">Payment Amount</span>
 {lp.isUnpaid ? (
 ...
 <span className="font-black text-foreground">{formatCompactNumber(lp.amount)}</span>
 )}
 </div>
 </div>
 </m.div>
 ...
 <div className="px-6 py-12 text-center text-muted-foreground font-medium italic text-sm">
 This customer has no history of late payments. (Perfect Score)
 </div>
 )}
 </div>
