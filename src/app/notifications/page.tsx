...
export default function NotificationsPage() {
 ...
 <h4 className={`text-sm font-bold ${notif.is_unread ?'text-foreground':'text-muted-foreground'}`}>
 {notif.title}
 </h4>
 <span className="text-[0.6875rem] font-medium text-muted-foreground">{timeAgo(notif.created_at || new Date().toISOString())}</span>
 </div>
 <p className={`text-sm leading-relaxed mt-1 ${notif.is_unread ?'text-muted-foreground':'text-muted-foreground'}`}>
 {notif.message}
 </p>
...
'bg-purple-600 text-white hover:bg-purple-700'
 ...
 />
 <span className="text-[10px] font-bold text-muted-foreground">of {totalPages}</span>
 </div>
 <div className="flex gap-2">
 <button 
 onClick={() => setPageMap(prev => ({ ...prev, [cat]: Math.max(1, currentPage - 1) }))}
 disabled={currentPage === 1}
 className="p-2 rounded-lg border border-border bg-card text-muted-foreground disabled:opacity-30 hover:bg-muted transition-colors"
