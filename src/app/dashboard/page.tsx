...
const RevenueTooltip = ({ active, payload, label }: TooltipProps) => {
 if (active && payload && payload.length) {
 return (
 <div className="bg-white px-4 py-3 rounded-xl shadow-xl border border-border">
 ...
 <span className="text-muted-foreground font-bold capitalize">{item.name}:</span>
 </span>
...
"Gamers Node":"#e0e3e5"
 ...
 <p className="text-muted-foreground font-bold tracking-wide">Loading Dashboard Data...</p>
 </div>
 </div>
 );
 }
 ...
 >
 <div>
 <h1 className="text-4xl font-bold text-foreground tracking-tight">Executive Overview</h1>
 ...
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Current Period</p>
 ...
 <p className="text-sm font-medium text-muted-foreground mt-1">Gross revenue vs projected expenses.</p>
 </div>
 <Link
 href="/profitability"
 className="
...
"
 ...
 <p className="text-sm font-medium text-muted-foreground mt-1">Segmentation</p>
 </div>
 ...
 <p className="text-[10px] font-bold text-muted-foreground uppercase mb-1">{payload[0].payload.month}</p>
 <p className="text-sm font-black text-foreground">{payload[0].value} Active</p>
 </div>
 );
 }
 return null;
 ...
 className="bg-card rounded-3xl p-8 shadow-2xl relative overflow-hidden group"
 >
 ...
 whileTap={{ scale: 0.95 }}
 className="w-full py-4 bg-white text-foreground rounded-2xl font-black text-sm hover:bg-muted transition-all flex items-center justify-center gap-2 relative z-10 shadow-xl shadow-white/5"
 >
