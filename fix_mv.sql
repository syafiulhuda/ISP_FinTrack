DROP MATERIALIZED VIEW IF EXISTS ar_aging_mv;
ALTER TABLE customers ALTER COLUMN "createdAt" TYPE TIMESTAMPTZ USING "createdAt"::TIMESTAMPTZ;

CREATE MATERIALIZED VIEW ar_aging_mv AS
WITH customerexpectedinvoices AS (
         SELECT c.id AS customer_id,
            c.village AS node,
            COALESCE(replace(replace(replace(st.price, 'Rp'::text, ''::text), ' '::text, ''::text), '.'::text, ''::text)::numeric, 0::numeric) AS expected_amount,
            generate_series((date_trunc('month'::text, (c."createdAt" AT TIME ZONE 'Asia/Jakarta'))::date + (EXTRACT(day FROM (c."createdAt" AT TIME ZONE 'Asia/Jakarta')) - 1::numeric)::integer)::timestamp with time zone, CURRENT_DATE::timestamp with time zone, '1 mon'::interval)::date AS virtual_due_date
           FROM customers c
             LEFT JOIN service_tiers st ON TRIM(BOTH FROM c.service) ~~* st.name
